...
public class MessageDao {
    ...
    public static void retrieveMessages(long conversationId, int limit, LinkedList<Message> messages, byte[] decryptionKey, byte[] senderEdPrivateKey, JdbcTemplate template) {
        String sql;
        if (messages.isEmpty()) {
            ...
            List<Message> fetchedMessages = template.query(
                    ...
                    (rs, rowNum) -> {
                        ...
                        byte[] signature = (signatureBlob != null) ? signatureBlob.getBytes(1, (int) signatureBlob.length()) : null;
            ...
            if (fetchedMessages != null && !fetchedMessages.isEmpty()) {
                for (Message message : fetchedMessages) { messages.addFirst(message); }
                System.out.println("Fetched " + fetchedMessages.size() + " message objects from the database");
            } else throw new IllegalStateException("No messages found for conversation ID: " + conversationId);
